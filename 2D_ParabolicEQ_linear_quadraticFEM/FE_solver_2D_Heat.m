function solution=FE_solver_2D_Heat(start_t,end_t,dt,theta,left,right,bottom,top,N1,N2,basis_type_trial,basis_type_test,Gauss_type,norm_type)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% this solver is based on Algorithm B(chapter 4,page 48)
% the stiffness matrix A is independent of time t.
% the load vector b is dependent of time t.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% mass matrix M, chapter 4, page 20.
M=assemble_matrix_2D('function_one',Gauss_type,basis_type_test,basis_type_trial,0,0,0,0,matrix_size,number_of_elements,P,T,Tb_test,Tb_trial,number_of_local_basis_fun_trial,number_of_local_basis_fun_test);
...
X_old=generate_initial_vector('function_initial',Pb);%X0

% Iterate in time
number_of_time_steps=(end_t-start_t)/dt;
for m=0:number_of_time_steps-1
    tm=m*dt;
    tmp1=(m+1)*dt;
    bm=assemble_vector_2D_t(tm,'function_f',number_of_elements,basis_type_test,0,0,b_vector_size,Tb_test,number_of_local_basis_fun_test,Gauss_type,P,T);
    bmp1=assemble_vector_2D_t(tmp1,'function_f',number_of_elements,basis_type_test,0,0,b_vector_size,Tb_test,number_of_local_basis_fun_test,Gauss_type,P,T);
    %chapter4, page 44
    b_tilde=theta*bmp1+(1-theta)*bm+A_fixed*X_old;
    [A_tilde,b_tilde]=treat_Dirichlet_boundary_2D_t(tmp1,'function_g',A_tilde,b_tilde,boundarynodes,Pb);
    solution=A_tilde\b_tilde;
    X_old=solution;
end
...
%here we only compute the error at the end time(t=end_t).
error=Error_type_2D_t(norm_type,end_t,solution,Pb,P,T,Tb_trial,Gauss_type,basis_type_trial)
